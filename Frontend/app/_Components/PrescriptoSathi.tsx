...
export default function PrescriptionMedicineFinder() {
  ...
  const findMedicineName = async () => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:5000/PrescriptoSathi", {
        method: "POST",
        body: formData,
      });
        ...
        <AnimatePresence mode="wait">
          ...
          {step === "results" && (
            <motion.div
              ...
              className="bg-white rounded-lg shadow-lg p-8 text-black max-w-lg mx-auto border border-[#006994]/40"
            >
              <h2 className="text-2xl font-semibold mb-4 text-[#004f63] text-center">Extracted Medicines</h2>

              {medicineName ? renderMedicineList() : (
                <p className="text-lg text-red-500">No medicine name found.</p>
              )}

              <button
                onClick={reset}
                className="mt-6 w-full px-3 py-1.5 rounded-xl font-medium border-2 border-cyan-600 bg-gradient-to-r from-[#004f63] to-blue-300 text-white hover:from-cyan-600 hover:to-cyan-700 transition-all"
              >
                Try Again
              </button>
            </motion.div>
